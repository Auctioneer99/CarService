			...
			<form class="mb-5 ml-0 row w-100" v-on:submit="onSearch">
				...
				<div class="form-group col-3 m-0">
					...
					<AutoComplete
						...
						:suggestions="form.filteredCustomers"
						@complete="searchCustomerTyped($event)"
						field="name"
...
export default {
	...
	methods: {
		searchCustomerTyped(event) {
			if (!this.form.customer || this.form.customer == "") {
				return;
			}
			var queryCustomers = `query{users(first:20 name:\"${this.form.customer}\" where:{role:{eq:\"customer\"}}){nodes{id name}}}`;
			graphQL
				.get(queryCustomers)
				.then((response) => {
					this.form.filteredCustomers = response.data.data.users.nodes;
				})
				.catch((error) => {
					this.form.filteredCustomers = [];
					console.log(error);
				});
		},
		onSearch(event) {
			...
			var queryReport = `query{workReportForCustomer(dateFrom:\"${this.form.dateFrom.toLocaleDateString(
				"en-US"
			...
			graphQL
				.get(queryReport)
				.then((response) => {
					this.report = response.data.data.workReportForCustomer ?? {
						price: 0,
						count: 0,
					};
				})
		...
		updateData(searchScheme, page) {
			...
			var queryWorks = `query{works(take:${
				...
				"en-US"
			)}\"}transport:{ownerId: {eq:\"${searchScheme.customer.id}\"}}})

	...
	public class AdoNetAppender : BufferingAppenderSkeleton
	{
		...
		virtual protected string GetLogStatement(LoggingEvent logEvent)
		{
			...
			else
			{
				...
				LogLog.Debug(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType, "LogStatement2 [" + writer.ToString() + "]");
		...
		virtual protected Type ResolveConnectionType()
		{
			try
			{
				return SystemInfo.GetTypeFromString(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType, ConnectionType, true, false);
			}
			catch (Exception ex)
			{
